---
layout: post title: "MongoDB--Linux环境下mongoDB分片集群部署"
category: 'MongoDB'
---
...
同样搭建一主一副本一仲裁，相关的配置文件、数据、日志都放在sharded_cluster相应的子目录下面， 具体步骤如下：
...
此时写入数据会报Database db could not be created :: caused by :: No shards
found，错误，原因是通过路由节点操作，现在只是连接了配置节点，还没有连接分片数据节点，因此无法写入业务数据。如下：
...
* 开启分片功能查看分片状态 sh.enableSharding("库名")、sh.shardCollection("库名.集合名",{"key":1})
...
* 集合分片 对于集合分片，使用 sh.shardCollection() 方法指定集合和分片键 语法：
